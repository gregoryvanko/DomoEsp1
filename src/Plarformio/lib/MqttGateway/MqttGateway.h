...
public:
     ...
     *
     * Le WiFiClient est géré en interne par la librairie.
     */
    MqttGateway(const char* brokerHost,
                uint16_t    brokerPort = 1883,
                const char* clientId   = "ESP32_Hydro");

    // PubSubClient garde une référence vers _wifiClient : l'objet ne doit pas être copié/déplacé
...
private:
    ...
    WiFiClient          _wifiClient; // Doit être déclaré avant _client (ordre d'initialisation)
